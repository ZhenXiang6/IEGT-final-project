...
\begin{frame}{求解結果－Steady State}
  \begin{columns}[T]
    ...
    \column{0.38\textwidth}\footnotesize
      25 條由不同起點 $s_0\in[0.02,0.98]$ 出發的模擬路徑\textbf{全部收斂到同一點}。
...
\begin{frame}{求解結果－轉移機制與在位者優勢}
  \begin{columns}[T]
    ...
    \column{0.44\textwidth}\footnotesize
      轉移 $s'=\rho(s)\,s+\alpha(s)(1-s)$ 可拆成兩股流入:
      ...
      \end{itemize}
      \vskip0.2em
      \begin{exampleblock}{非顯然結果:兩種網路外部性方向相反}
        留存端 $\kappa_\rho$ \textbf{鞏固既有者}(穩態升、市場集中);獲取端 $\kappa_\alpha$ 反而\textbf{助挑戰者追趕}($\kappa_\alpha$ 由 $0\!\to\!0.6$,incumbent advantage 由 $\sim$28\% 降至 $\sim$16\%)。
      \end{exampleblock}
  \end{columns}
\end{frame}
