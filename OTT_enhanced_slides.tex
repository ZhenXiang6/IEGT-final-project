...
\begin{frame}{模型建構－單期決策流程}
  ...
  \begin{itemize}\footnotesize
    \item 每期先觀察市佔 $s_t$,再從 4 種策略擇一 $(P_t,C_t)$,賺取當期利潤後,經舊客流失與新客加入決定 $s_{t+1}$。
    \item \textbf{關鍵}:$s_t$ 在當期決策時\textbf{給定},$s_{t+1}$ 則由本期策略\textbf{內生}形成 —— 故須以動態最佳化求解,而非逐期靜態。
  \end{itemize}
\end{frame}
...
\begin{frame}{模型建構－狀態轉移(市佔率)}
  ...
  \end{itemize}
  \vskip0.3em
  {\footnotesize 下面先以\textbf{水桶比喻}直觀說明,後續再給出 $\rho$、$\alpha$ 與利潤的函數形式}
\end{frame}

\begin{frame}{模型建構－市佔率怎麼動(水桶比喻)}
  ...
  \end{center}
  \vskip0.4em
  {\footnotesize\centering 直覺:\textbf{獲取}(\textcolor{ottteal}{$\alpha$})把水倒進桶、\textbf{留存}(\textcolor{rblue}{$\rho$})決定漏掉多少,市佔即\textbf{水位}。\par}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{block}{Strategy $\Rightarrow$ Retention rate}
      \begin{itemize}\itemsep1pt
        \item $\delta_S{>}0$:訂閱制(S)對 Retention 有額外加成。{\color{ottgray}Iyengar et al.\ 2011:含計次成分費率使留存 $-10.5\%$。}
        \item $\delta_R{>}0$:留存策略(推薦/原創/防共享)提升留存。{\color{ottgray}Ben Rhouma \& Zaccour 2018。}
...
\begin{frame}{模型建構－顧客獲取率(Acquisition rate $\alpha$)}
  ...
  \begin{columns}[T]
    ...
    \column{0.54\textwidth}\footnotesize
    \begin{block}{Strategy $\Rightarrow$ Aqquisition rate}
      \begin{itemize}\itemsep1pt
        \item $\tilde\delta_T{>}0$:計次(T)無訂閱承諾、門檻低 $\Rightarrow$ 拉新較易。{\color{ottgray}Datta et al.\ 2015(計次/免費試用提高獲取)。}
...
\begin{frame}{模型建構－Revenue \& Cost function}
  ...
  {\footnotesize\textbf{$\lambda_A>\lambda_R$:}市場越飽和,獲取(A)成本隨市佔率\textbf{陡升}、留存(R)\textbf{無此規模效應} $\Rightarrow$ 廠商提早轉向留存(Min et al.\ 2016)。\par}
  \vskip0.3em
...
\begin{frame}{結論}
  ...
  \begin{exampleblock}{方法論貢獻}
    在同一 Bellman 框架下整合\hl{收費模式選擇}與\hl{客群策略},並讓收費模式 $P$ 同時影響 $\rho$ 與 $\alpha$;。
  \end{exampleblock}
\end{frame}
