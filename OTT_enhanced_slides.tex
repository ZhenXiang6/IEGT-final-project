...
\begin{frame}{延伸模型1：臨界規模}
  ...
  \end{center}
  \vskip0.15em
  {\footnotesize\centering 起點 $s_0<0.79\Rightarrow$ 0.53;\quad $s_0>0.79\Rightarrow$ \textbf{0.93(獨佔)}。\par}
\end{frame}

\begin{frame}{延伸模型1：解讀}
  \begin{block}{為什麼?}\footnotesize
    \begin{itemize}\itemsep4pt
...
\begin{frame}{延伸模型2：競爭模型}
  \begin{columns}[T]
    ...
    \begin{tikzpicture}[font=\scriptsize,
      ...
      \node[box,fill=rblue!12,draw=rblue] (N) at (0,1.7) {大公司\\$s_N$};
      \node[box,fill=ottred!12,draw=ottred] (C) at (4.6,1.7) {新進公司\\$s_C$};
    ...
    \begin{block}{設定}
      \begin{itemize}\itemsep2pt
        \item 兩家每期都有「游移客」$W{=}\omega s$ 可被對方挖;\textbf{大公司的比例較高} $\omega_N{=}0.15>\omega_C{=}0.075$。
        \item 兩家也從\textbf{未訂閱池}拉新;大公司跌破 0.54 由留存轉獲取。
      \end{itemize}
    \end{block}
    \vskip0.2em
    {\scriptsize $s_N'{=}(\rho_N{+}0.10)s_N+\alpha_N U-\alpha_C\omega_N s_N+\alpha_N\omega_C s_C$\par
...
\begin{frame}{延伸模型2：結果}
  \begin{columns}[T]
    ...
    \begin{block}{$\omega_N{=}0.15$}
      \begin{itemize}\itemsep3pt
        \item 大公司 $0.7\!\to\!\mathbf{0.54}$、新進公司 $0\!\to\!\mathbf{0.31}$(\textbf{被侵蝕但仍領先})。
        \item \textbf{新進公司策略}:市佔小、\textbf{全程獲取}—TA$\to$ SA ;大公司則一直SR不變。
      \end{itemize}
    \end{block}
    \begin{exampleblock}{游移比例($\omega_N$)的變化}
      游移比例越高、差距越小:\\$\omega_N{=}0.05$ gap 0.58、$0.15$ gap 0.24、$0.30$ 幾乎平手。
    \end{exampleblock}
  \end{columns}
\end{frame}
...
\begin{frame}{延伸模型2:解讀}
  ...
  \begin{itemize}\itemsep5pt
    \item 大公司較高的游移比例讓新進挖到客、站穩腳跟。
    \item 游移比例($\omega_N$) 越高,領先差距越小。
    \item 沒有游移($\omega{=}0$)新進無法搶佔市場($\sim$0.06)。
  \end{itemize}
  \vskip0.3em
  \begin{alertblock}{與現實的觀察和連接}\footnotesize
    解釋 Netflix 為何死守\textbf{降低顧客游移}(原創內容、個人化)，市佔率不應為公司所注重的唯一指標，仍需注重降低顧客的游移比例。
  \end{alertblock}
\end{frame}
